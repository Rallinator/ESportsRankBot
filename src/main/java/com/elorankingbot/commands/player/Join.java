...
import static com.elorankingbot.model.MatchFinderQueue.QueueType.PREMADE;
import static com.elorankingbot.model.MatchFinderQueue.QueueType.SOLO;
import static com.elorankingbot.timedtask.TimedTask.TimedTaskType.LEAVE_QUEUES;
    ...
    protected void execute() {
        ...
        if(game.getRoleId() != 0) {
            ...
            if(member != null) {
                ...
                if(roles != null){
                    for (Role role : roles){
                        if(role.getId().asLong() == game.getRoleId()){
                            matched = true;
                        }
                    }
                }
                else {
                    event.createFollowup("User does not have the role required to join queue.").withEphemeral(true).subscribe(NO_OP, asyncExceptionCallback());
                    return;
                }
            }
            if(!matched){
